      ...
      for (int nz=0; nz<nx3; ++nz) {
        ...
        for (int m=0; m<=nmb1; ++m) {
          ...
          if ((x1 >= x1min && x1 <= x1max) &&
              ...
              iindcs.h_view(nx,ny,nz,1) = static_cast<int>(std::floor((x1-(x1min+dx1/2.0))/dx1));
              iindcs.h_view(nx,ny,nz,2) = static_cast<int>(std::floor((x2-(x2min+dx2/2.0))/dx2));
              iindcs.h_view(nx,ny,nz,3) = static_cast<int>(std::floor((x3-(x3min+dx3/2.0))/dx3));
          }
        }
      }
    }
